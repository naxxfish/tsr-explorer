...
async function processTsrMessage (tsr, batchMessage) {
  let version = 1
  try {
  ...
  try {
    ...
    throw new Error(`Couldn't insert TSR record: ${e} - ${JSON.stringify(tsr)}`)
  }
}
...
function handleTSRBatchMsgV1 (tsrMessage) {
  ...
  const tsrMessagePromises = []
  batchMessage.tsr.forEach((tsr) => {
    tsrMessagePromises.push(processTsrMessage(tsr, batchMessage))
  })
  Promise.allSettled(tsrMessagePromises).then((results) => {
    let processedMessageCount = 0
    results.forEach((result) => {
      if (result.status === 'fulfilled') {
        processedMessageCount++
      } else if (result.status === 'rejected') {
        console.error(`TSR Message rejected: ${result.reason}`)
      }
    })
    console.log(`Processed ${processedMessageCount} TSRs successfully`)
  })
}
